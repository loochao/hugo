...
* `{{ %` as the outer-most shortcode delimiter means "pass the inner content to the content renderer" (e.g. Blackfriday);  the old behavior can be had, see  https://github.com/gohugoio/hugo/issues/5763
...
* The taxonomy nodes now have a `.Page` accessor which makes it much simpler to get a proper `.Title` etc. see https://github.com/gohugoio/hugo/issues/5719.
* The template keywords `with` and `if` now work properly for zero and interface types, see https://github.com/gohugoio/hugo/issues/5739
...
* A new configuration option, `Permalinkable`, is added to Output Format and enabled that by default for `HTML` and `AMP` types. See  https://github.com/gohugoio/hugo/issues/5706 
...
* Chroma is updated with many fixes and new lexers, see https://github.com/gohugoio/hugo/issues/5797
* We now support `Page`-relative aliases, e.g. aliases that do not start with a `/`, see https://github.com/gohugoio/hugo/issues/5757
* We now support context-relative (language) URLs in front matter, meaning that in most cases `url: /en/mypage` can be replaced with the more portable `url: mypage`. See https://github.com/gohugoio/hugo/issues/5704
