...
[Custom Output Formats](https://gohugo.io/templates/output-formats) has been a really useful feature, but it has had some annoying and not so obvious restrictions that is now lifted. Now all `Page` collections are aware of the output format being rendered. This means, to give some exapmples, that:
...
If you want the old behaviour, you can put the following line in the start of your shortcode template:
